...
class TransferForegroundService : Service() {
    ...
    fun bindTelemetry(
            telemetryFlow:
                    kotlinx.coroutines.flow.StateFlow<
                            List<com.fluxsync.core.protocol.ChannelTelemetry>>
    ) {
        telemetryJob?.cancel()
        telemetryJob =
                serviceScope.launch {
                    telemetryFlow.collect { telemetry ->
                        val wifi =
                                telemetry
                                        .firstOrNull {
                                            it.type == com.fluxsync.core.protocol.ChannelType.WIFI
                                        }
                                        ?.weightFraction
                                        ?: 0f
                        val usb =
                                telemetry
                                        .firstOrNull {
                                            it.type ==
                                                    com.fluxsync.core.protocol.ChannelType.USB_ADB
                                        }
                                        ?.weightFraction
                                        ?: 0f
                        val speed =
                                telemetry
                                        .fold(0L) { acc, it -> acc + it.throughputBytesPerSec }
                                        .toFloat() / 1_048_576f

                        val notification = buildTransferNotification(wifi, usb, speed)
    ...
    fun buildTransferNotification(
        ...
        val remoteViews =
                RemoteViews(packageName, R.layout.notification_transfer).apply {
                    setTextViewText(
                            R.id.tv_speed,
                            String.format("%.1f MB/s", speedMbs.coerceAtLeast(0f))
                    )
    ...
    private fun createNotificationChannels() {
        ...
        val transferChannel =
                NotificationChannel(
                        ...
                        .apply {
                            description =
                                    "Shows ongoing FluxSync transfer speed and channel balance"
                            setShowBadge(false)
                        }
